...
const CharacterV1 = ({
  ...
  const x = useTransform(scrollYProgress, [0, 1], [distanceFromCenter * 15, 0]);
  const rotateX = useTransform(scrollYProgress, [0, 1], [distanceFromCenter * 15, 0]);
  const opacity = useTransform(scrollYProgress, [0, 0.4, 1], [0.2, 0.8, 1]);

  return (
    <motion.span
      className={cn("inline-block", isSpace && "w-3 sm:w-4")}
...
export interface AnimatedScrollHeadingProps {
  text: string;
  className?: string;
}

export function AnimatedScrollHeading({ text, className }: AnimatedScrollHeadingProps) {
  ...
  return (
    <h2
      ref={targetRef}
      className={cn("tracking-tight font-extrabold text-slate-950", className)}
